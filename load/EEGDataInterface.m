...
% as well as ica source separation (source component plot)
    ...
    methods
       ...
       function obj = set_name(obj, dataset_name, data_dir) 
            ...
            obj.data_path = [fileparts(fileparts(mfilename('fullpath'))) '/processed_data/' data_dir '_Data'];
            mkdir(obj.data_path);
        ...
        function faster_load_raw(obj, reading_file)
            ...
            cur_folder_path = fileparts(mfilename('fullpath'));
            % TODO  remove this for testing only this is used for emotiv only
            channel_file = [cur_folder_path, '/emotive_channel_info.ced'];
            ...
            EEG = eeg_checkset( EEG );
            EEG = pop_saveset( EEG, 'filename', ['pre.set'],'filepath',[cur_folder_path, '/Faster_Processing/pre']);

            % use options
            load([cur_folder_path, '/Faster_Processing/faster_options.mat'])
            FASTER(option_wrapper)

            % load the set into EEGDataInterface and save them 
            obj.curEEG = pop_loadset([cur_folder_path, '/Faster_Processing/post/pre.set']);
            % unepoch data
        ...
        function obj = load_raw(obj, readings_file)
            ...
            channel_file = [cur_dirname '/' 'emotive_channel_info.ced']
            obj.curEEG = LoadEEGData(readings_file, obj.dataset_name, channel_file, obj.data_path)%,'no fancy rejection');% can change to raw dataset only
            obj = obj.extract_EEG_data();
        end
